...
int main(int argc, char *argv[])
{
    ...
    if (argc > 1) {
        ...
        printf("vector :%s", argv[1]);
        strcpy((char*)fileName, (const char*)argv[1]);
    }
    else {
        printf("Please add the output file name as parameter\n");
        exit(exitCode);
    }
...
float getAccelVector() {
    ...
    if(fileName) {
        printf ("file name %s\n", fileName);
        FILE* testerFile = fopen(fileName, "a");
...
void refreshAccelerometer() {  
    ...
    if ( (acceleration < ACCEL_THRESHOLDLOW) || (acceleration > ACCEL_THRESHOLDUP) && acceleration < ACCEL_LIMIT) { 
...
void getDataNN() {
	...
	if (accelBuffer[sampleCount] > ACCEL_LIMIT) {
		accelBuffer[sampleCount] = accelBuffer[sampleCount - 1];
	}
    sampleCount++;
...
void sendEdisonPackage(char event_type)
{
	...
	strcpy (char_tmp, (const char*)" 20.673 ");
    strcat(system_python_call_str, (const char*)char_tmp);
    strcpy (char_tmp, (const char*)"103.335 ");
